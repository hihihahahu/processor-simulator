...
void iplc_sim_LRU_replace_on_miss(int index, int tag)
{
	//if it is a direct map
	if (cache_assoc == 1)
	{
		//directly set the tag and vb.
		cache[index].assoc[0].tag = tag;
		cache[index].assoc[0].vb = 1;

	}
	//if it is not a direct map
	else {
		int i = 0;
		//get the oldest element number.
		int temp = cache[index].replacement[0];
		//update the index and tag in for the oldest element
		cache[index].assoc[temp].vb = 1;
		cache[index].assoc[temp].tag = tag;
		//then we can percolate everything up
		for (i = 0; i < cache_assoc - 1; i++)
		{
			cache[index].replacement[i] = cache[index].replacement[i + 1];
		}
		//set the recent used one to the oldest element.
...
void iplc_sim_LRU_update_on_hit(int index, int assoc)
{
	int i = 0, j = 0;
	int temp;
	//create a s for size of the replacement vector.
	int s = cache_assoc;
	//elseif it is a direct map do nothing
	if (s > 1)
	{
		//firstly, find the position if assoc in replacement
		for (i = 0; i < s; i++)
		{
			if (cache[index].replacement[i] == assoc)
				temp = i;
		}
		//secondly, percolate everything after assoc
		for (j = temp + 1; j < s; j++)
		{
			cache[index].replacement[j - 1] = cache[index].replacement[j];
		}
		//finally, set the recent used one to assoc.
		cache[index].replacement[s - 1] = assoc;
...
int iplc_sim_trap_address(unsigned int address)
{
    ...
    //for loop for check every element.
	for (i = 0; i < cache_assoc; i++) {
		//if we can hit
		if (cache[index].assoc[i].tag == tag && cache[index].assoc[i].vb) {
			//if it is not a directed map.
			if (cache_assoc > 1) {
				//update the information by using update on hit.
    ...
    //if we cant hit, so plus the miss count.
	cache_miss++;
	//update the data by using replace on miss
...
void iplc_sim_push_pipeline_stage()
{
	int i;
	int data_hit = 1;
	int inserted_nop = 0;//0 for not inserted, 1 for inserted
	int branch_taken = 0;//0 for not taken, 1 for taken
	...
	if (pipeline[DECODE].itype == BRANCH)//if we need to branch
	{
		if (pipeline[FETCH].instruction_address)//if FETCH is not a bubble
		{
			/*
			if the address of the instruction in FETCH is not 4 bits greater
			than the address of instruction in DECODE, there must have taken branches
			*/
			if (((int32_t)pipeline[FETCH].instruction_address - (int32_t)pipeline[DECODE].instruction_address) != 4)
			{
				branch_taken = 1;
			}
			//if the branch prediction not equals to the result,prediction fails
			if (branch_taken != branch_predict_taken)
			{
				++pipeline_cycles;//stall for one cycle caused by false prediction

				//pushing the pipeline and add a nop
				...
				//"Count WRITEBACK stage is 'retired'" for the new cycle
				if (pipeline[WRITEBACK].instruction_address)
				{
					++instruction_count;
				}
			}
			else//increment variable for correct prediction
			{
				++correct_branch_predictions;
			}
		}
		++branch_count;//increment branch count
	}

	/* 3. Check for LW delays due to use in ALU stage and if data hit/miss add delay cycles if needed.*/
	if (pipeline[MEM].itype == LW)
	{
		//get data hit in case if there is a halt
		data_hit = iplc_sim_trap_address(pipeline[MEM].stage.lw.data_address);

		//if r-type, it may need register that return by the load instruction
		//if the destination reg is same as the reg 1 or 2 of the r-type,
		//we need to wait for the load instruction,
		//need to stall for one cycle, since forwording won't solve this problem
		...
		{
			//we need increment one cycle,load instruction and add nop in MEM
			++pipeline_cycles;
			memcpy(&pipeline[WRITEBACK], &pipeline[MEM], sizeof(pipeline_t));
			...
			inserted_nop = 1;//help later calculation,indicated one cycle already added
		}

		//if there is a miss, add total cycles by stated in CACHE_MISS_DELAY
		if (data_hit == 0)
		{
			if (inserted_nop == 1)
			{
				//decrement 1 for the cycle already added in the code above
				//decrement 1 for later cycle increment in part 5
		...
		//other related data hazard solved by forwarding
	}

	/* 4. Check for SW mem acess and data miss .. add delay cycles if needed */
	if (pipeline[MEM].itype == SW)
	{
		//we have the same miss penalty for miss as write in the project
	...
	{
		memcpy(&pipeline[i], &pipeline[i - 1], sizeof(pipeline_t));
	}
